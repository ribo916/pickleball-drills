...
export function buildInteractiveCourtSVG(positions, stepIdx) {
  let s = '';
  ...
  // Click-target overlay (transparent rects on every cell)
  for (let r = 0; r < GRID_ROWS; r++) {
    for (let c = 0; c < GRID_COLS.length; c++) {
      const coord = `${GRID_COLS[c]}${r + 1}`;
      const x = COURT_PAD_X + c * CELL_W;
      const y = COURT_PAD_TOP + r * CELL_H;
      s += `<rect x="${x}" y="${y}" width="${CELL_W}" height="${CELL_H}" fill="transparent" style="cursor:pointer" onclick="setPlayerPosition(${stepIdx},'${coord}')"/>`;
    }
  }

  // Player tokens (drawn on top of click targets so they're visible)
  PLAYER_LABELS.forEach((label, i) => {
    const coord = positions && positions[label];
    if (!coord) return;
    const pos = gridToXY(coord);
